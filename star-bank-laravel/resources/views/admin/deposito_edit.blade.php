...
<head>
    ...
    <title>Edit Tipe Deposito - Star Bank</title>
    <style>
        body {
...
<body>
    <div class="modal-box">
        ...
        <h2 style="margin-bottom: 30px;">Edit Tipe Deposito</h2>
        
        <form action="/admin/deposit/update/{{ $depositoType->id }}" method="POST">
            @csrf
            @method('PUT')

            <div class="form-group">
                <label>ID Deposito</label>
                <input type="text" value="{{ $depositoType->id }}" disabled style="background: #f5f5f5; color: #888;">
            </div>

            <div class="form-group">
                <label>Nama Deposito</label>
                <input type="text" name="name" value="{{ $depositoType->name }}" required>
            </div>
            
            <div class="form-group">
                <label>Bunga Tahunan (%)</label>
                <input type="number" step="0.01" name="yearly_return" value="{{ $depositoType->yearly_return }}" required>
            </div>
            ...
            <div style="display: flex; gap: 15px; margin-top: 20px;">
                ...
                <button type="submit" class="btn-update" style="flex: 1;">Simpan</button>
            </div>
        </form>
    </div>
</body>
